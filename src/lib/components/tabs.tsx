...
interface TabProps { title: string, id: string, disabled?: boolean, children: ComponentChildren }
...
interface TabsProps { active: string, onChangeTab: (id: string) => void }
export default class Tabs extends Component<TabsProps> {
  ...
  render (): ComponentChildren {
    ...
    return <TabInterface active={active} onChangeTab={onChangeTab}>{this.children}</TabInterface>
  }
}
...
const TabInterface = WithID<TabInterfaceProps>(class TabInterface extends Component<TabInterfaceProps> {
  private readonly handleTabChange = (id: string, event: MouseEvent): void => {
  ...
  private readonly findTab = (id: string): TabProps | null => {
    const candidate = this.props.children.find((child): child is TabChild & { type: 'tab' } => {
      ...
      <div class={styles.container}>
        <ul role='tablist' class={styles.list}>
          {children.map((child, index) => {
            if (child.type === 'label') {
              return <li key={index} class={styles.label}><Label {...child} /></li>
            }
            ...
            return <li role='tab' onClick={handleClick} id={tabID} tabindex={0} key={index} aria-selected={selected} aria-disabled={disabled} class={clz} aria-controls={panelID}>{title}</li>
          })}
        </ul>
        ...
        {children.map((child, index) => {
          ...
          return <div role='tabpanel' key={index} id={panelID} aria-labelledby={tabID} class={clz}>{childNodes}</div>
        })}
      </div>
    )
  }
