...
describe(ImmutableMap, () => {
  test('has(), get(), size, keys(), values() and entries() work on a new map', () => {
    ...
    expect(Array.from(mp.entries())).toEqual([
      [1, 'a'],
      [2, 'b'],
      [3, 'c'],
    ])
    expect(Array.from(mp)).toEqual([
      [1, 'a'],
      [2, 'b'],
      [3, 'c'],
    ])
  ...
  test('set() and setAll() create a new map with appropriate entries', () => {
    ...
    expect(Array.from(unchanged.entries())).toEqual([
      [1, 'a'],
      [2, 'b'],
      [3, 'c'],
    ])
    ...
    expect(Array.from(add4.entries())).toEqual([
      [1, 'a'],
      [2, 'b'],
      [3, 'c'],
      [4, 'd'],
    ...
    expect(Array.from(replace3.entries())).toEqual([
      [1, 'a'],
      [2, 'B'],
      [3, 'c'],
    ])
    ...
    const replaceMore = original.setAll([
      [1, 'A'],
      [2, 'B'],
      [3, 'C'],
      [4, 'D'],
    ])
    expect(Array.from(replaceMore.entries())).toEqual([
      [1, 'A'],
      [2, 'B'],
      [3, 'C'],
      [4, 'D'],
    ...
    const laterWins = original.setAll([
      [1, 'A'],
      [2, 'B'],
      [3, 'C'],
      [4, 'D'],
    ...
    expect(Array.from(laterWins.entries())).toEqual([
      [1, 'aa'],
      [2, 'B'],
      [3, 'C'],
      [4, 'D'],
  ...
  test('delete() and deleteAll() delete entries and creates an independent map', () => {
    ...
    expect(Array.from(deleteNonExisting.entries())).toEqual([
      [1, 'a'],
      [2, 'b'],
      [3, 'c'],
    ])
    ...
    expect(Array.from(deleteExisting.entries())).toEqual([
      [1, 'a'],
      [2, 'b'],
    ])
    expect(Array.from(original.entries())).toEqual([
...
describe(ImmutableMapWithDefault, () => {
  test('has(), get(), size, keys(), values() and entries() work on a new map', () => {
    const mp = new ImmutableMapWithDefault('a', [
      [1, 'a'],
      [2, 'b'],
      [3, 'c'],
    ])
  ...
  test('set() and setAll() creates a new map with appropriate entries', () => {
    const original = new ImmutableMapWithDefault('d', [
      [1, 'a'],
      [2, 'b'],
      [3, 'c'],
    ])
    ...
    expect(Array.from(add5.entries())).toEqual([
      [1, 'a'],
      [2, 'b'],
      [3, 'c'],
    ])
  ...
  test('delete() and deleteAll() delete entries and creates an independent map', () => {
    const original = new ImmutableMapWithDefault('a', [
      [1, 'a'],
      [2, 'b'],
      [3, 'c'],
    ])
    ...
    expect(Array.from(deleteExistingDefault.entries())).toEqual([
      [2, 'b'],
      [3, 'c'],
    ])
    expect(Array.from(original.entries())).toEqual([
